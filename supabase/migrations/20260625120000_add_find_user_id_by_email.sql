...
--
-- Se compara `email` exacto (no `lower(email)`) para usar el indice nativo de
-- `auth.users` sobre `email`: GoTrue normaliza los emails a minuscula al
-- crearlos y el codigo ya invita/busca en minuscula. No se crea un indice
-- propio porque `auth.users` es una tabla gestionada por Supabase y el rol de
-- migracion no puede alterarla.
